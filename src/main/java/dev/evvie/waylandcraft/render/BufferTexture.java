...
public abstract class BufferTexture {
	...
	public static abstract class BasicBufferTexture extends BufferTexture {
		...
		public BasicBufferTexture(int width, int height, int format) {
			...
			GlTexture glTexture = IGlTextureMixin.createTexture(GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING, "buffertexture-" + this.hashCode(), GpuFormat.RGBA8_UNORM, width, height, 1, 1, id, frameBufferCache());
...
//			if(stride % 4 != 0) WaylandCraft.LOGGER.info("Stride is not a multiple of 4 bytes!!");
	...
	private static com.mojang.blaze3d.opengl.FrameBufferCache frameBufferCache() {
		return ((IGlDeviceMixin) ((IGpuDeviceMixin) RenderSystem.getDevice()).getBackend()).invokeFrameBufferCache();
	}
	...
	public static class DmabufTexture extends BufferTexture {
		...
		private void init() {
			...
			GlTexture glTexture = IGlTextureMixin.createTexture(GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING, "eglimage-" + this.hashCode(), GpuFormat.RGBA8_UNORM, width, height, 1, 1, eglImageTex, frameBufferCache());
